document.addEventListener('DOMContentLoaded', () => {
  ...
  const api = {
    ...
    update: (id, payload) => fetch(`/api/customerlicense/${encodeURIComponent(id)}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest', 'X-CSRF-TOKEN': csrf }, body: JSON.stringify(payload) }).then(r => r.ok ? r.json() : r.text().then(t => { throw new Error(t); })),
    del: (id, hard = false) => fetch(`/api/customerlicense/${encodeURIComponent(id)}?hard=${hard}`, { method: 'DELETE', headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest', 'X-CSRF-TOKEN': csrf } }).then(r => r.ok ? r.json() : r.text().then(t => { throw new Error(t); })),
    create: payload => fetch(`/api/customerlicense`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest', 'X-CSRF-TOKEN': csrf }, body: JSON.stringify(payload) }).then(r => r.ok ? r.json() : r.text().then(t => { throw new Error(t); }))
  };
  let dt = null;
