document.addEventListener('DOMContentLoaded', () => {
  const api = {
    ...
    update: (id, payload) => fetch(`/api/customerlicense/${encodeURIComponent(id)}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }).then(r => r.ok ? r.json() : r.text().then(t => { throw new Error(t); })),
    del: (id, hard = false) => fetch(`/api/customerlicense/${encodeURIComponent(id)}?hard=${hard}`, { method: 'DELETE' }).then(r => r.ok ? r.json() : r.text().then(t => { throw new Error(t); })),
    create: payload => fetch(`/api/customerlicense`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }).then(r => r.ok ? r.json() : r.text().then(t => { throw new Error(t); }))
  };
  let dt = null;
  ...
  async function voApi(url, method, body) { const res = await fetch(url, { method: method || 'GET', headers: { 'Content-Type': 'application/json' }, body: body ? JSON.stringify(body) : undefined }); const txt = await res.text(); try { return { ok: res.ok, code: res.status, json: JSON.parse(txt) }; } catch { return { ok: res.ok, code: res.status, text: txt }; } }
